...
dtable_clear_crop_data <- function(dat, no_y=16) {
...
dtable_add_prev_crop_type <- function(dat){
  pctype <- dat$CType
...
dtable_add_prev_prev_crop_type <- function(dat){
  ppctype <- dat$CType
...
dtable_read_crop <- function(data_ref, exclude_grass=FALSE, exclude_crops=FALSE) {
  ...
  data_crop <- dtable_clear_crop_data(data_crop, 16)
  
  data_crop$PCType <- dtable_add_prev_crop_type(data_crop)
  data_crop$PPCType <- dtable_add_prev_prev_crop_type(data_crop)
